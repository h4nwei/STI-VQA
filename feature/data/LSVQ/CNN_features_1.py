...
def main(dataset, features_folder):
    ...
    for i in range(len(dataset)):
        ...
        if video_name.split('/')[0] == 'ia-batches':
            saved_name = os.path.join(features_folder, video_name.split('/')[2], video_name.split('/')[3])
            save_folder = os.path.join(features_folder, video_name.split('/')[2])
        else:
            saved_name = os.path.join(features_folder, video_name.split('/')[1], video_name.split('/')[2])
        ...
        feat_mu_std = get_features(current_data, frame_batch_size=2)
...
if __name__ == "__main__":
    ...
    features_folder = r'/home/zhw/vqa/dataset/LSVQ/feature/VSFA_resnetxt101_iqapretrain_ms'
    video_folder = r'/data/zhw/LSVQ'
    csv_file = Path('/data/zhw/LSVQ/LSVQ_1080p.csv')
    ...
    for _, folder_name in enumerate(os.listdir(features_folder)):
        for index, exist_name in enumerate(os.listdir(os.path.join(features_folder, folder_name))):
            # ind = sub_video_names.index(exist_name[:-4])
            name = folder_name+'/'+exist_name[:-4]
            if name in sub_video_names:
                sub_video_names.remove(name)
    print('exsiting {} videos'.format(len(sub_video_names)))
    video_names = []
    
    for video_name in sub_video_names:
        head_name = video_name.split('/')[0]
        if 'ia-batch' in head_name:
            video_names.append(os.path.join('ia-batches', video_name.split('/')[0], video_name))
        else:
            video_names.append(os.path.join(video_name.split('/')[0], video_name))
