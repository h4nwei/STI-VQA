...
def feature_difference_momentum(features):
    b, n, c = features.shape
    features = features[:, :, :c//2]
    diff_feature_1 = features[:, 1:] - features[:, :-1]
    last_feature_1 = torch.zeros_like(features[:, 0:1], device=features.device, dtype=torch.float32)
    diff_feat_1 = torch.cat([last_feature_1, diff_feature_1], dim=1)
    r1 = diff_feat_1

    diff_feature_2 = features[:, 2:] - features[:, :-2]
    last_feature_2 = torch.zeros_like(features[:, 0:2], device=features.device, dtype=torch.float32)
    diff_feat_2 = torch.cat([last_feature_2, diff_feature_2], dim=1)

    diff_feature_3 = features[:, 3:] - features[:, :-3]
    last_feature_3 = torch.zeros_like(features[:, 0:3], device=features.device, dtype=torch.float32)
    diff_feat_3 = torch.cat([last_feature_3, diff_feature_3], dim=1)
    r2 = (diff_feat_1 + diff_feat_2 + diff_feat_3)/3

    diff_feature_4 = features[:, 4:] - features[:, :-4]
    last_feature_4 = torch.zeros_like(features[:, 0:4], device=features.device, dtype=torch.float32)
    diff_feat_4 = torch.cat([last_feature_4, diff_feature_4], dim=1)

    diff_feature_5 = features[:, 5:] - features[:, :-5]
    last_feature_5 = torch.zeros_like(features[:, 0:5], device=features.device, dtype=torch.float32)
    diff_feat_5 = torch.cat([last_feature_5, diff_feature_5], dim=1)
    r3 = (diff_feat_1 + diff_feat_2 + diff_feat_3 + diff_feat_4 + diff_feat_5) / 5
    
    return torch.cat([r1, r2, r3], dim=2)
...
class ViT(nn.Module):
    def __init__(self, *, input_dim=4096, mlp_dim=128, dim_head=64, output_channel=1, depth=5, heads=6, pool = 'reg', dropout = 0.1, emb_dropout = 0.1, max_length=1000):
        ...
        self.reduce_embedding_0 = nn.Linear(1280, mlp_dim, bias=False)
        self.reduce_embedding_1 = nn.Linear(2560, mlp_dim, bias=False)
        self.reduce_embedding_2 = nn.Linear(5120, mlp_dim, bias=False)
        self.reduce_embedding_3 = nn.Linear(10240, mlp_dim, bias=False)
    ...
    def forward(self, video, video_len, max_len):
        ...
        x = torch.cat((x, feature_difference_momentum(x)), dim=2)
        ...
        x1 = video[1]
        x1 = torch.cat((x1, feature_difference_momentum(x1)), dim=2)
        ...
        x2 = video[2]
        x2 = torch.cat((x2, feature_difference_momentum(x2)), dim=2)
        ...
        x3 = video[3]
        x3 = torch.cat((x3, feature_difference_momentum(x3)), dim=2)
