...
class Linear_cd(nn.Module):
    def __init__(self, diff_channels, kernel_size, bias=False, theta=0.5):

        super(Linear_cd, self).__init__() 
        self.cov1d = nn.Conv1d(in_channels=diff_channels, out_channels=diff_channels, kernel_size=kernel_size, stride=1, padding=kernel_size//2)
        self.theta = theta
...
class Embedding(nn.Module):
    def __init__(self, input_dim, red_dim, step, max_len):
        ...
        self.fc_diff = Linear_cd(diff_channels=max_len, kernel_size=step)
...
class ViT(nn.Module):
    def __init__(self, *, input_dim=4096, mlp_dim=128, dim_head=64, output_channel=1, depth=5, heads=6, pool = 'reg', dropout = 0.1, emb_dropout = 0.1, max_length=1000):
        ...
        self.reduce_embedding = nn.Linear(input_dim*2, mlp_dim, bias=False)
...
def test_VQATransformer():
    ...
    video_len[0, 0] = 5
    video_len[1, 0] = 8
    vqa = ViT(input_dim=8, mlp_dim=4, dim_head=2, output_channel=1, depth=2, heads=2, pool = 'reg', dropout = 0.1, emb_dropout = 0.1, max_length=10).cuda()
