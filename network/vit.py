...
class ViT(nn.Module):
    def __init__(self, input_size=448, dim=512, depth=2, heads=16, mlp_dim=128, dim_head=64, dropout=0.1, emb_dropout=0.1):
        super().__init__()
        ...
        self.patch_to_embedding = nn.Linear(self.chn_sum, dim)
    ...
    def forward(self, x, batch_avg=False):
        if len(x.size()) == 5:
            batch_n, patch_n, c, h, w = x.size()
            batch_avg = True
            x = x.reshape(batch_n*patch_n, c, h, w)

        h1s = self.forward_once(x)
        hyperfeat_lists = []
        for h1 in h1s[:]:
